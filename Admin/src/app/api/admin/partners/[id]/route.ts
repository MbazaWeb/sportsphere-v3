...
export async function GET(request: NextRequest, ctx: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  const { data } = await supabaseAdmin.from('ss_partner').select('*').eq('id', id).limit(1);
...
export async function PATCH(request: NextRequest, ctx: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  const { data, error } = await supabaseAdmin.from('ss_partner').update(body).eq('id', id).select('*').maybeSingle();
  if (error) return NextResponse.json({ error: error.message, data: null });
  return NextResponse.json({ data });
}

export async function DELETE(request: NextRequest, ctx: { params: Promise<{ id: string }> | { id: string } }) {
  const auth = await requireAdmin(request);
  if (!auth.authorized) return auth.response;
  const { id } = await Promise.resolve(ctx.params as any);
  await supabaseAdmin.from('ss_partner').delete().eq('id', id);

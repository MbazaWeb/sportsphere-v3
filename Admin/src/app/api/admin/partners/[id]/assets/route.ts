...
export async function GET(request: NextRequest, ctx: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  const rows = await ssList('ss_partner_assets', (q) => q.eq('partner_id', id).limit(100));
...
export async function POST(request: NextRequest, ctx: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  const { data, error } = await supabaseAdmin.from('ss_partner_assets').insert({ id: crypto.randomUUID(), partner_id: id, ...body }).select('*').maybeSingle();

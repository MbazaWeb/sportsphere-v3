...
FIXTURES = [
    ...
    ("Coastal Union FC","Geita Gold FC","2027-03-07","18:00"),("Dodoma FC","Kagera Sugar","2027-03-07","20:30"),
    ...
    ("Polisi Tanzania FC","Namungo","2027-04-18","16:00"),("Kagera Sugar","Coastal Union FC","2027-04-18","19:00"),
...
# Build SQL
lines = []
lines.append("\\set ON_ERROR_STOP")
lines.append("BEGIN;")
lines.append("")
...
for home, away, date_str, time_str in FIXTURES:
    ...
    if time_str == "TBA":
        kickoff = f"{date_str} 18:00:00+03"
    else:
...
# Save locally
with open('/home/z/my-project/scripts/seed-nbc.sql', 'w') as f:
    f.write(sql)
...
with open('/home/z/my-project/scripts/seed-nbc.sql', 'rb') as local:
    sftp.putfo(local, '/tmp/seed-nbc.sql')
sftp.close()

# Execute via SSH
stdin, stdout, stderr = client.exec_command(
    "PGPASSWORD='SS_Secure_2024!' psql -U sportsphere_admin -d sportsphere -f /tmp/seed-nbc.sql 2>&1 | tail -5",
    timeout=120
)
...
for q in [
    ...
    'SELECT "homeTeam", "awayTeam", "kickoffAt" FROM "Match" ORDER BY "kickoffAt" LIMIT 3;'
]:

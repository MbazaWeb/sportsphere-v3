...
export function SportlightsTab({ onShare, onComment }: SportlightsTabProps) {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      {/* Live Match Card (same as before, truncated for brevity) */}
      {featuredMatch && (
      ...
      {/* ── THE FIXED SPOTLIGHT VIDEO SECTION ── */}
          ...
          <div className="flex flex-col gap-3">
            {spotlightItems.map((item) => (
              <div key={item.id} className="video-card-container bg-[#1e2126] rounded-2xl p-4 mb-0 border border-surface-border">
                ...
                {/* 1. Caption goes inside the container first */}
                <div className="mb-3">
                  <p className="text-white text-sm font-medium">{item.content}</p>
                </div>

                {/* 2. Video goes inside the container below the caption */}
                ...
                {/* Like & Comment stats inside the container */}

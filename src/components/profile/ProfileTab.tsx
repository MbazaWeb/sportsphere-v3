...
function LoggedInProfile({ onNavigate, onLogout }: { onNavigate: (section: string) => void; onLogout: () => void; }) {
  ...
  const [activeTab, setActiveTab] = useState<'spotlight'>('spotlight');
  ...
  const profileTabs = [{ id: 'spotlight' as const, label: 'Spotlight' }];
      ...
      <div className="p-4">
        ...
        <div className="mb-6 flex items-start gap-4">
          <div className="relative">
            <div className={cn('flex h-20 w-20 items-center justify-center rounded-full border-2 text-2xl font-bold overflow-hidden', isVerified ? 'border-gold bg-gold text-black' : isPending ? 'border-yellow-400 bg-yellow-500/10 text-yellow-400' : 'border-gold bg-surface-elevated text-gold')}>
            ...
            <button className="absolute bottom-0 right-0 flex h-7 w-7 items-center justify-center rounded-full bg-gold"><Camera className="h-3.5 w-3.5 text-black" /></button>
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap"><h2 className="text-xl font-bold text-white">{userProfile?.name || 'User'}</h2>{isVerified && <ShieldCheck className="h-5 w-5 text-gold" />}</div>
            <p className="text-sm text-muted-foreground">{userProfile?.handle || '@user'}</p>
            <div className="mt-2 flex items-center gap-2 flex-wrap"><BadgeStack role={userProfile?.role || 'fan'} isVerified={isVerified} typeName={userProfile?.typeName} size="sm" /></div>
            <button onClick={() => setEditOpen(true)} className="mt-2 rounded-lg bg-surface border border-surface-border px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-surface-elevated">Edit Profile</button>
            {canUpgrade && (
              <button onClick={() => setUpgradeOpen(true)} className="mt-2 ml-2 rounded-lg bg-gradient-to-r from-gold to-yellow-500 px-3 py-1.5 text-xs font-bold text-black transition-all hover:shadow-[0_4px_20px_rgba(245,197,24,0.3)] active:scale-[0.98]"><Sparkles className="mr-1 inline h-3 w-3" /> Upgrade to PRO</button>
            )}
          </div>
        </div>
        ...
        <div className="mb-6 grid grid-cols-3 gap-3 rounded-2xl bg-surface-elevated border border-surface-border p-4">
          <div className="text-center"><p className="text-lg font-bold text-white">{userProfile?.followerCount ?? 0}</p><p className="text-[10px] text-muted-foreground uppercase">Followers</p></div>
          <div className="text-center"><p className="text-lg font-bold text-white">{userProfile?.followingCount ?? 0}</p><p className="text-[10px] text-muted-foreground uppercase">Following</p></div>
          <div className="text-center"><p className="text-lg font-bold text-white">{userProfile?.postCount ?? 0}</p><p className="text-[10px] text-muted-foreground uppercase">Posts</p></div>
        </div>

        {userProfile?.sportsFollowing && userProfile.sportsFollowing.length > 0 && (
          <div className="mb-6"><h3 className="mb-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Sports Following</h3><div className="flex flex-wrap gap-2">{userProfile.sportsFollowing.map((sport) => <span key={sport} className="rounded-xl bg-gold/10 px-3 py-1.5 text-xs font-medium text-gold border border-gold/20">{sport}</span>)}</div></div>
        )}

        <div className="mb-4 flex gap-1">{profileTabs.map((tab) => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={cn('flex-1 rounded-lg py-2 text-sm font-semibold transition-colors', activeTab === tab.id ? 'bg-gold text-black' : 'bg-surface text-muted-foreground hover:text-foreground')}>{tab.label}</button>))}</div>

        {/* ====== UNIFIED SPOTLIGHT FEED ====== */}
        <div className="flex flex-col gap-3">
          {postsLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-10"><Zap className="h-8 w-8 text-muted-foreground/30 mb-2" /><p className="text-sm text-muted-foreground">No spotlight content yet</p></div>
          ) : (
            realPosts.map((post) => (
              <article key={post.id} className="rounded-2xl bg-surface-elevated border border-surface-border overflow-hidden glass-card-hover">
                ...
                <div className="p-4 border-b border-surface-border/30 flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-white">{userProfile?.name || 'User'}</p>

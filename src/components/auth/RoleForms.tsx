...
export default function RoleForms({ role, data, onChange }: RoleFormsProps) {
  ...
  switch (role) {
    case 'team':
      return (
        <div className="flex flex-col gap-4">
          <SelectField label="Sport" placeholder="Select primary sport" value={data.sport || ''} onChange={(v) => update('sport', v)} options={SPORTS_OPTIONS} />
    ...
    case 'player':
      return (
        <div className="flex flex-col gap-4">
          <SelectField label="Sport" placeholder="Select your sport" value={data.sport || ''} onChange={(v) => update('sport', v)} options={SPORTS_OPTIONS} />
    ...
    case 'journalist':
      return (
        <div className="flex flex-col gap-4">
          <SelectField label="Primary Beat" placeholder="Select your coverage area" value={data.beat || ''} onChange={(v) => update('beat', v)} options={[
            { value: 'transfers', label: 'Transfer News' }, { value: 'match-reporting', label: 'Match Reporting' },
            { value: 'analysis', label: 'Analysis & Opinion' }, { value: 'investigative', label: 'Investigative' },
    ...
    case 'analyst':
      return (
        <div className="flex flex-col gap-4">
          <SelectField label="Analysis Type" placeholder="Select your focus" value={data.analysisType || ''} onChange={(v) => update('analysisType', v)} options={[
            { value: 'data-stats', label: 'Data & Statistics' }, { value: 'tactical', label: 'Tactical Analysis' },
            { value: 'performance', label: 'Performance Analysis' }, { value: 'predictive', label: 'Predictive Modeling' }
          ]} />
    ...
    case 'creator':
      return (
        <div className="flex flex-col gap-4">
          <SelectField label="Content Type" placeholder="Select your primary content" value={data.contentType || ''} onChange={(v) => update('contentType', v)} options={[
            { value: 'highlights', label: 'Match Highlights' }, { value: 'analysis', label: 'Analysis & Breakdown' },
            { value: 'comedy', label: 'Comedy & Entertainment' }, { value: 'news', label: 'News & Updates' },
            { value: 'vlogs', label: 'Vlogs & Behind the Scenes' }, { value: 'podcasts', label: 'Podcasts' }
          ]} />
    ...
    case 'scout':
      return (
        <div className="flex flex-col gap-4">
          <SelectField label="Scouting Type" placeholder="Select your specialization" value={data.scoutType || ''} onChange={(v) => update('scoutType', v)} options={[
            { value: 'youth', label: 'Youth Talent' }, { value: 'first-team', label: 'First Team Recruitment' },
            { value: 'opposition', label: 'Opposition Analysis' }, { value: 'free-agents', label: 'Free Agent Monitoring' }
          ]} />
    ...
    case 'organization':
      return (
        <div className="flex flex-col gap-4">
          <SelectField label="Organization Type" placeholder="Select type" value={data.orgType || ''} onChange={(v) => update('orgType', v)} options={[
            { value: 'federation', label: 'Sports Federation' }, { value: 'governing-body', label: 'Governing Body' },
            { value: 'association', label: 'Sports Association' }, { value: 'ngo', label: 'NGO / Charity' },
    ...
    case 'business':
      return (
        <div className="flex flex-col gap-4">
          <SelectField label="Business Type" placeholder="Select type" value={data.businessType || ''} onChange={(v) => update('businessType', v)} options={[
            { value: 'sportswear', label: 'Sportswear / Equipment' }, { value: 'media', label: 'Sports Media' },
            { value: 'agency', label: 'Sports Agency' }, { value: 'nutrition', label: 'Sports Nutrition' },
            { value: 'tech', label: 'Sports Technology' }, { value: 'merchandise', label: 'Merchandise / Memorabilia' },
